...
params:
  ...
  id: [710761126,710661010,710561118,710661119]
  group: "teamE"
  url: ""
---
...
此處$Y_{1i}$為可通過限制學生的個經PR，$Y_{0i}$為沒有通過限制的學生的個經PR，兩個相減為具有通過限制資格學生的個經PR效果。
</td></tr></table>
...
<table><tr><td bgcolor=#F5DEB3>
i泛指任何人，此處$Y_{i}$泛指學生的個經PR，$Y_{1i}$為可通過限制學生的個經PR，$Y_{0i}$為沒有通過限制的學生的個經PR，兩個相減為具有通過限制資格學生的個經PR效果。
$(Y_{1i}-Y_{0i})T_i$代表通過限制的學生（T=1），該學生的個經PR為未通過限制學生的效果加上通過限制學生的效果。反之，若未通過限制的學生（T=0），則該學生的個經PR為只有未通過限制學生的效果。
</td></tr></table>
...
<table><tr><td bgcolor=#F5DEB3>
立足點高的人（即經原PR高）比較傾向認真上個經，而立足點低的人（經原PR低）比較傾向不認真上個經，另外還存在微積分這個遺漏變數，所以造成上題的估計結果可能不正確。
</td></tr></table>
...
<table><tr><td bgcolor=#F5DEB3>
若控制了經原PR，則可以確定「會不會修個經」和「個經PR」之間為獨立，可以排除因為經原PR高，而容易選擇修個經，進而獲得較高的個經PR的狀況。
</td></tr></table>
...
hw1Data %>%
  group_by(可修個體) %>%
  summarise(經原PR平均=mean(經原PR,na.rm=T),個經PR平均=mean(個經PR,na.rm=T))
```
...
請填入以下空格完成完整偏誤論述：  
<table><tr><td bgcolor=#F5DEB3>
有控制**經原PR**時，擋修會使得**個經PR** (1)_下降_(上升/下降) (2)_6.802_個 百分點，其值比未控制**經原PR**時還
(3)_低_(高/低)；這表示忽略**經原PR**會對效應係數估計產生 (4)_正向_(正向偏誤(upward bias)/負向偏誤(downward bias))。之所以會有上述的偏誤方向來自於兩個因素：
...
&=\sum_{i,x_i=1}^N (1-\frac{N_1}{N})(y_i-\bar{y})+{\sum_{i,x_i=0}^N(0-\frac{N_1}{N})(y_i-\bar{y})}\\
&=\frac{N_0}{N}\sum_{i,x_i=1}^N(y_i-\bar{y})-\frac{N_1}{N}\sum_{i,x_i=0}(y_i-\bar{y})\\
&=\frac{N_0}{N}\sum_{i,x_i=1}^Ny_i-\frac{N_0N_1}{N}\bar{y}-\frac{N_1}{N}\sum_{i,x_i=0}^Ny_i+\frac{N_0N_1}{N}\bar{y}\\
&=\frac{N_0}{N}\sum_{i,x_i=1}^Ny_i-\frac{N_1}{N}\sum_{i,x_i=0}^Ny_i----(1)\\
\\
&再證明分母：\\
&\sum_{i=1}^N (x_i-\bar{x})^2\\
&=\sum_{i,x_i=1}^N(1-\frac{N_1}{N})^2+\sum_{i,x_i=0}^N(0-\frac{N_1}{N})^2\\
&=N_1(\frac{N_0}{N})^2+N_0(\frac{N_1}{N})^2\\
&=\frac{N_1N_0}{N^2}(N_0+N_1)=\frac{N_1N_0}{N^2}N\\
&=\frac{N_1N_0}{N}----(2)\\
&將(1)式與(2)式相除可得出：\\
&\hat{\beta_1}=\frac{\frac{N_0}{N}\sum_{i,x_i=1}^Ny_i-\frac{N_1}{N}\sum_{i,x_i=0}^Ny_i}{\frac{N_1N_0}{N}}\\
&=\frac{\sum_{i,x_i=1}^Ny_i}{N_1}-\frac{\sum_{i,x_i=0}^Ny_i}{N_0}\\
&=\bar{y_1}-\bar{y_0}
\end{align}
</td></tr></table>
...
\begin{align}
&(a)\hat{\beta_1}\\
&=\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})(y_i-\bar{y})}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}\\
&=\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})y_i}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}\\
&=\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})(\beta_0+\beta_1x_i+\epsilon_i)}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}\\
&=\beta_1\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})x_i}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}+\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})\epsilon_i}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}\\
&=\beta_1+\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})\epsilon_i}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}\\
&E(\hat{\beta_1}|x)=E(\beta_1+\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})\epsilon_i}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}|x)\\
&因為\epsilon_i|x_i)=0,所以E(\hat{\beta_1}|x)=\beta_1+0\\
&故E(\hat{\beta_1})=\beta_1\\
&(b)V(\hat{\beta}_1|x))\\
&=V(\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})\epsilon_i}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}|x)\\
&=E[(\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})\epsilon_i}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2})^2|x]-[E(\frac{\sum_{i,x_i=1}^N(x_i-\bar{x})\epsilon_i}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}|x)]^2\\
&=\frac{1}{[\sum_{i,x_i=1}^N(x_i-\bar{x})^2]^2}\sum_{i,x_i=1}^NE(x_i-\bar{x})^2\epsilon_i^2]x\\
&=\frac{\sigma^2}{\sum_{i,x_i=1}^N(x_i-\bar{x})^2}\\
&分母套入2.1分母的計算結果\\
&=\frac{\sigma^2}{\frac{N_0N_1}{N}}\\
&=\frac{N\sigma^2}{N_0N_1}\\
&(c)V(\hat{\beta}_1|x)\\
&=V(\bar{y_1}-\bar{y_0}|x)\\
&=V(\bar{y_1})+V(\bar{y_0})-2cov(y_1,y_0)\\
&套入2.2(b)的計算結果\\
&=\frac{N_1}{N_1^2}\sigma_1^2+\frac{N_0}{N_0^2}\sigma_0^2\\
